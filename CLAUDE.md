...
- Imagens de post: `python3 scripts/gera_imagem.py --listar` mostra os presets de formato (feed, feed-retrato, story, capa-reel, linkedin, youtube-thumb, og) e os modelos. **A `GEMINI_API_KEY` do ambiente tem cota 0 para modelos de imagem** (429 em todos), então hoje o caminho que funciona é `--backend openai` (gpt-image-2). Camada de estilo da marca fica em `assets/marca/estilo.json` (ainda não existe: identidade a definir no FRAMEWORK.md, o script avisa e usa base neutra).
